...
file.sources = list.files(c("/Users/h/Documents/project_local/cue_expectancy/scripts/step02_R/utils"),
                          pattern="*.R", 
                          full.names=TRUE, 
                          ignore.case=TRUE)
...
main_dir <- "/Users/h/Dropbox/projects_dropbox/iclr-2023/third_revision/"
model = 'resnet50'
concat_df = data.frame()
  ...
  if (model == "alexnet") {
    ...
    data_fname <- file.path(main_dir, 'data', 'resnet50', paste0(dv, '-mobilenet.csv'))
...
# average_fisherz$classTMP <- average_fisherz$layer
# average_fisherz$layerTMP <- average_fisherz$class
# average_fisherz$layer <- average_fisherz$layerTMP
# average_fisherz$class <- average_fisherz$classTMP
average_fisherz$func  = factor(average_fisherz$functionality)
average_fisherz$layer_ordered <- factor(average_fisherz$layer, 
                                   levels=c( "layer1.0",  "layer1.1", "layer1.2",
...
p3 <- ggplot(data = subjectwise,
  ...
  # scale_fill_manual(values = int_color, guide = "none") +
  # scale_color_manual(values = int_color,  guide = "none") +
