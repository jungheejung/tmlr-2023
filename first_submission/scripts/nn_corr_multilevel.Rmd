...
```{r contrast code}
average_fisherz$perf_con[average_fisherz$functionality == "ablation_impact"] <- -0.5
average_fisherz$perf_con[average_fisherz$functionality == "decoding_accuracy_delta"] <- 0.5

average_fisherz$perf  = factor(average_fisherz$functionality)
...
full = lmer(corr_z ~ perf_con * metric_con + (1|class)  , data = average_fisherz)
summary(full)
```


```{r DEP}
full = lmer(corr_transform ~ perf_con * metric_con + (perf_con * metric_con|class)  + (perf_con * metric_con|layer) , data = concat_df)
summary(full)
```
```{r}
...
se = "se"
subject = "class"
ggtitle = "main effect of functionality across layers and classes"
...
guides(color = "none") +
  ...
  ggtitle(ggtitle) +
  # scale_x_discrete(
  #                   limits=c("causal deficit","non-causal decoding"))+
  scale_x_discrete(
    labels = c(
...
se = "se"
subject = "class"
ggtitle = "main effect of metric across layers and classes"
legend_title = "metric"
xlab = "metric"
ylab = "correlation"
w = 5
...
group_mean = "mean_per_sub_norm_mean" 
se = "se" ; subject = "class"
ggtitle = "interaction of functionality and metric" 
