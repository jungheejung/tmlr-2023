...
concat_df = data.frame()
  for (dv in c('ablation_impact_X_cka', 
    ...
    if (model == "alexnet") {
      data_fname <- file.path(main_dir, 'first_submission', 'data', model, paste0(dv, '.csv'))
    } else if (model == "mobilenet") {
      data_fname <- file.path(main_dir, 'first_submission', 'data', model, paste0(dv, '-mobilenet.csv'))
    }
...
save_fname <-
  file.path(main_dir,'first_submission',
            'results',
            'figure',
