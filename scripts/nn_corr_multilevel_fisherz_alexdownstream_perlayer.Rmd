...
Here, we average correlation values as a function of functionality X metric per class
In other words, we're averaging over the layers
...
average_fisherz <- meanSummary(concat_df,
                         c("class", "func", "metric", "layer"), "fisherz")
...
model.tst <- lmer(fisherz ~ func_con * metric_con + layer + (1 |class), data = concat_df )
summary(model.tst)
```
...
model_keyword = "alexnet_downstream"
...
for (layer_name in c("features.1"  , "features.4"  , "features.7"   ,"features.9",   "features.11" , "classifier.2","classifier.5")) {
subset <- average_fisherz[average_fisherz$layer == layer_name, ]
classwise <- meanSummary(subset,
                         c(subject, iv ), "fisherz")
...
ggtitle = paste(model_keyword, layer_name,
